...
Router.post("/send-qr-code", (req, res, next) =>
    (async (req, res) => {
        ...
        if (validationResult.status === 0) throw new AppError(validationResult.message, 402);

        const message = getMessageText(req.body);

        const codeResponse = await generateQR(message);
        if (codeResponse.status === 0) throw new Error(codeResponse.message);
        ...
        if (messageResponse.status === 0) throw new Error(messageResponse.message);
